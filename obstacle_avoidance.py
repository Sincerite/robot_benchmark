...
while robot.step(timeStep) != -1:
    ...
    _in = 1.5
    _ext = 1.5
    _left_sens  = (outerLeftSensorValue/_ext + centralLeftSensorValue*_in)
    _right_sens = (outerRightSensorValue/_ext + centralRightSensorValue*_in) - centralSensorValue*_in
    
    if _left_sens == 0 and _right_sens == 0:
        _left_sens = _left_sens + (values[0] ** 2) / values[0]
        
    _left = max(-9.53, min(initialVelocity - _right_sens , maxMotorVelocity))
    _right = max(-9.53, min(initialVelocity - _left_sens  , maxMotorVelocity))
